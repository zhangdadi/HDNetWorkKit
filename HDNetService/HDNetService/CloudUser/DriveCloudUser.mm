...
@interface DriveCloudUser ()
{
    iNetLib::ocNotifyPointer _updateLogOut;
    ocNotifyPointer _updateLoadUserInfo;
}
@end
...
-(id)init
{
    ...
    _updateLoadUserInfo[self] = @selector(loadUserInfoUpdate);
    _updateLoadUserInfo.Receive(_userInfoCtrl.onUpdate);

    
    _logOutCtrl = [[DCDataCtrl_LogOut alloc] init];
    _logOutCtrl.param.user = self;
    _updateLogOut[self] = @selector(updateLogOut);
    _updateLogOut.Receive(_logOutCtrl.onUpdate);
...
- (void)updateLogOut {
    if (_logOutCtrl.netError == nil) {
        [self setNoneData];
    }
}

- (void)loadUserInfoUpdate {
    if (_userInfoCtrl.data != nil && _userInfoCtrl.data.errorMessage == nil) {
        [self setUserData];
    }
}
