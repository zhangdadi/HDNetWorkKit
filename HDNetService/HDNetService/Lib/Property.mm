...
namespace{	namespace OCDataTransferFunc{
    ...
    // Dictionary
    //	将ObjC数据字典赋值给Object
    static void Dictionary(id Object,NSDictionary *MapDict,NSDictionary *OCData){
        if(OCData==nil)
            return;
    ...
    static id CreateDictionary(id Value,Class ObjectClass,NSDictionary *ObjectDict)
    {
        if(Value==nil)
            return nil;
        if([Value isKindOfClass:[NSDictionary class]]==false){
            DebugLog(@"OCDataTransfer错误：字段不是Dict\n");
            return nil;
        }
        id DestObject=SafeCreateObject(ObjectClass);
        Dictionary(DestObject,ObjectDict,Value);
        return DestObject;
    }
...
void iNetLib::OCDataTransfer(id Object,NSDictionary *ObjectPropertyMap,id OCData)
{
    ...
    if(PSetter!=nil){
        ...
        if(PName==nil || PName.length==0){
            // 对应到自身
            if([SrcDataObject isKindOfClass:[NSArray class]]){
                // 如果是数组,取第一个数据
                auto *Array=static_cast<NSArray*>(SrcDataObject);
                if(Array.count==0)
                    SrcDataObject=nil;
                else
                    SrcDataObject=Array[0];
            }
...
void iNetLib::_OCDataTransferDict(id Object,NSDictionary *ObjectPropertyMap,id OCData)
{
    if(OCData==nil)
        return;
    NSString *PName=ObjectPropertyMap[@"name"];
    Class PClass=[NSDictionary class];
    NSDictionary *PMap=ObjectPropertyMap[@"dict"];
    id PSetter=ObjectPropertyMap[@"setter"];
